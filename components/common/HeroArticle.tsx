...
interface HeroArticleProps {
  article: {
    ...
    author_name: string;
    published_at: string;
    // Added optional href to support your mock data structure if needed
    href?: string; 
  };
}

export default function HeroArticle({ article }: HeroArticleProps) {
  // Fallback if your data uses 'href' instead of constructing it from slug
  const articleLink = article.href || `/articles/${article.slug}`;

  return (
    <section className="mb-12 border-b border-slate-200 pb-12">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10">
        
        {/* 1. IMAGE COLUMN (Spans 8 of 12 columns on large screens) */}
        <div className="lg:col-span-8">
          <Link href={articleLink} className="group block relative h-[300px] md:h-[450px] overflow-hidden rounded-sm">
            <Image 
              ...
              unoptimized 
            />
            {/* "Breaking" Label overlay on the image */}
        ...
        {/* 2. TEXT COLUMN (Spans 4 of 12 columns) */}
        <div className="lg:col-span-4 flex flex-col justify-center">
          <div className="mb-4">
             <Badge label={article.category.name} />
          </div>

          <Link href={articleLink} className="group block mb-4">
